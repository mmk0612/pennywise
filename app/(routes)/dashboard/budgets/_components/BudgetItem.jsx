...
function BudgetItem({ budgetInfo, onDelete, readOnly = false }) {
  ...
  return (
    <div className="relative p-5 border rounded-lg hover:shadow-md h-[170px] bg-white">
      {readOnly ? (
        <div className="block h-full pr-10">
          <BudgetBody budgetInfo={budgetInfo} calculateProgressPercentage={calculateProgressPercentage} />
        </div>
      ) : (
        <Link
...
function BudgetBody({ budgetInfo, calculateProgressPercentage }) {
  return (
    <>
        <div className="flex items-center gap-2 justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {budgetInfo.billingMonth ? (
                <h2 className="text-xs text-slate-400">{budgetInfo.billingMonth}</h2>
              ) : null}
            </div>
          </div>
